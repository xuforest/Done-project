/*
	...
	links    empty   //An array containing list of links for every image.
		...
		function _genHtml(body_content){
			...
			body_content.each(function(){
				...
				if (settings.images.length) {
					...
					if (ext != 'swf') {
						var ind = 0;
						var lin = 0;  // 创建用于存放外链的变量lin
						for(i = 0; i < settings.images.length; i++){
							if (settings.transitionEffect == 'simpleSlide' && settings.sequenceMode == 'back') {
								ind = settings.images.length-1-i;
								lin = settings.links.length-1-i;
							}
									else { ind = i; lin=i;}
							if ($.fn.bgStretcher.settings.preloadImg) {
									code += '<li><span class="image-path">' + settings.images[ind] + '</span></li>';
								} else {
									code += '<li class="img-loaded"><a href="'+ settings.links[lin] +'" target="_blank"><img src="' + settings.images[ind] + '" alt="" /></a></li>';
								}		
						}
					} else {
